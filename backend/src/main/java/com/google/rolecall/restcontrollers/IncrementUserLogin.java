...
public class IncrementUserLogin extends AsyncRestEndpoint{
  ...
  @Get
  public CompletableFuture<Integer> getUserLoginCounter(@RequestParam(value="userid", required=true) int id) {
    Optional<User> queryResult = userRepo.findById(id);
    if (!queryResult.isPresent()) {
      return CompletableFuture.failedFuture(new InvalidArgumentException(String.format("userid %d does not exist", id)));
    } 
  ...
  @Post
  public CompletableFuture<Integer> postIncrementUserLogin(@RequestParam(value="userid", required=true) int id) {
    Optional<User> queryResult = userRepo.findById(id);
    if (!queryResult.isPresent()) {
      return CompletableFuture.failedFuture(new InvalidArgumentException(String.format("userid %d does not exist", id)));
    } 
